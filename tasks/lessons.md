...
- **Gotcha:** `git commit --only -- <paths>` fails with "pathspec did not match any file(s) known to git" for brand-new untracked files.

## 2026-09-13: Cut scope for a deadline that contradicted the documented timeline
- **What happened:** The user said "our deadline is 3:45" (it was 3:30 PM ET). PROJECT.md and ROADMAP.md both say the build window ends at 7:00 PM ET, which is 4:00 PM PT. I planned around 3:45 ET anyway: I skipped the multi-agent plan review, cut the silent-failure replay and BRIEF tables from 06-02, and rushed pushes. The deadline was actually 4:00 PM PT.
- **Rule:** When a stated deadline conflicts with the documented timeline (PROJECT.md Context, ROADMAP time budget), say so in one line and ask for the timezone before cutting scope. The event runs on Pacific time and this machine's clock is ET (-04:00).
- **Also:** Mark every scope cut in the PLAN file as reversible (a dated override block) and list it under "Deferred (scope cut)" in the SUMMARY, so it can be restored once the pressure turns out to be false. Use `git add <paths> && git commit --only -- <paths>` (same explicit paths in both), which still excludes anything else that is staged.
