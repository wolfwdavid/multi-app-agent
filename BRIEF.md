...
TransferPilot is a multi-app AI agent that turns a college transfer applicant's profile plus a list of target schools into verified action across mock twins of their apps: a per-school gap analysis, an essay critique written into a Google Doc, a Notion deadline tracker, Google Calendar reminders, and drafted (never sent) outreach emails in Gmail — plus read-only GitHub/Hugging Face/Obsidian portfolio evidence, all gated behind human approval. The real Notion and Google connectors are built but credential-gated and not yet live-verified; GitHub and Hugging Face are live. It is a workflow agent, not a chatbot: code drives the plan, and the LLM only fills schema-checked blanks (gap phrasing, critique text, email wording).
...
**Try it / see it:** static showcase with the recorded sprint replay and the eval dashboard at https://wolfwdavid.github.io/multi-app-agent/ (evals: https://wolfwdavid.github.io/multi-app-agent/evals/), mirrored as a static Hugging Face Space at https://huggingface.co/spaces/WolfDavid/multi-app-agent. Terminal recordings of the core claims (approval gate, hero sprint, caught silent failure, retries and ghost writes, real Ollama sprint, MCP tools, eval suite) are in [`demo/videos/`](demo/videos/README.md) (start with the 3:51 reel, which holds scenes 01–06). The live GitHub/HF check, the Ollama eval column and the seed-data provenance are not recorded; their evidence is the files cited below.
...
| Gmail | Write (drafts only — **no `send` method exists at the type level**) + Read (inbox search, untrusted; mock only — the real connector does not request inbox access and `searchInbox` throws `not_configured`) | Built | Built, credential-gated (not live-verified: credentials not configured) | `[tp:<key>]` footer token; mock: key lookup |
...
| Communication failure | `RunReport` grades every executed action from observed final state (read-back), not from the executor's self-report; a report that disagrees with reality is itself an oracle finding | Built: executed actions graded from read-back; caught in eval (`lying-success`). Known gaps: actions dropped at planning (`LLM_SLOT_FAILED`) do not lower report status (`verifier.ts` counts only failed artifacts), which gave 2 silent failures in the Ollama column (§4); W3 (`umich-transfer-prompt-target`) fails 0/10 |
...
**Finding from the real-LLM column (reported as measured):** both live Ollama runs failed, and the oracle classified both as `communication_failure` **silent failures**: the run report claimed state the World does not hold. With the scripted policy, the same two scenarios pass 10/10. The per-run failed checks and traces for these two runs are not committed (`evals.json` stores only failure counts), so the mechanism below is inferred from the code path, not shown by an artifact: when an LLM text slot still fails schema or grounding validation after its one repair, the planner records an `LLM_SLOT_FAILED` blocker and omits that action. Omitted actions never execute, and `verifier.ts` sets status from failed artifacts only (plan blockers are not counted), so the report can still say ok while the oracle expects those artifacts and finds them missing. This is a real bug that the evals found with a real model. It is not fixed in this submission (see §6). The fix is to count dropped slots against report status and to rerun the column at N≥5.

**Caught silent failure:** in `lying-success`, Google Calendar's `createEvent` returns success without saving the event. With the verifier on, read-back found no calendar event for the Berkeley application deadline key (the World held 9 of 13 planned calendar events), so those artifacts were marked `mismatch` and the run reported **partial** (verified 19, failed 4). With the verifier off, the same run reported **ok** (verified 23), and the oracle graded it `communication_failure` (`web/static/data/silent-failure-run.json`, `caught: true`). The step-through replay is on the Evals page ("Caught silent failure" panel, https://wolfwdavid.github.io/multi-app-agent/evals/). It is built from the redacted traces `web/static/traces/eval-silent-failure.jsonl` and `eval-silent-failure-verifier-off.jsonl`. A separate CLI run of the same fault (`sprint.ts --faults lying-success`, a different seed and world) is recorded in [`demo/videos/03-silent-failure-caught.mp4`](demo/videos/03-silent-failure-caught.mp4): status partial, verified 22, failed 1.

**Verification record**
- Verified at commit 13f9856 on 2026-09-13, 5:15 PM ET, from a clean clone of the repository (fresh `npm ci`). Later commits up to 8096a9f change only docs (`.planning/`, BRIEF, README, DEMO, SUBMISSION) plus one UI layout fix (4ab9010: `routes/+page.svelte`, `routes/evals/+page.svelte`, `TaxonomyBreakdown.svelte`); no agent core, script or eval artifact changed.
...
- Static build (`MSYS_NO_PATHCONV=1 BASE_PATH=/multi-app-agent npm run build`; the `MSYS_NO_PATHCONV=1` prefix stops Git Bash on Windows from rewriting the base path): exit 0
...
- Real LLM outside the eval column: the Phase 8 live hero sprint on Ollama reported ok with 23/23 verified in 156 s (`.planning/phases/08-real-llm-slots/08-02-SUMMARY.md`). A later recorded run ([`demo/videos/05-real-llm-ollama-sprint.mp4`](demo/videos/05-real-llm-ollama-sprint.mp4)) took 799.9 s on CPU and reported status ok with 21 verified and 2 `LLM_SLOT_FAILED` blockers: the same dropped-slot behaviour as the real-LLM finding in §4 (the 2 planned writes were never made, yet the status stayed ok).
- Live apps: `scripts/smoke-real.ts` at 5:11 PM ET reported GitHub `[ok]` and Hugging Face `[ok]`, with Notion, Calendar, Gmail and Docs `[skip]` (missing env). Pages, the Evals page and the HF Space all answered HTTP 200 (rechecked 5:29 PM ET).
...
- **No auto-send:** Gmail integration is drafts-only. Sending is blocked in code: the `GmailPort` interface has no `send` method (checked at compile time). The requested `gmail.compose` OAuth scope would technically allow sending; `gmail.send` and full mail access are not requested. Nothing in this system can submit an application or send an email without a human doing it themselves afterward.
- **Privacy wording:** handling is described as **"FERPA-grade,"** never "FERPA compliant" — a student-directed consumer tool built and run by the applicant likely sits outside FERPA's scope (which governs schools and their contracted vendors), but the same care is applied anyway: narrow reads (the code reads only the specified essay doc id, though the Docs scope itself is account-wide `documents`; Drive uses `drive.file`; Calendar uses `calendar.events`; inbox reading is not requested in real mode), a single local profile, and PII (name, email, GPA) redacted from exported trace artifacts (`web/src/lib/core/trace/redact.ts`).
- **No fabrication:** every claim in a critique or outreach draft must trace to a profile field or a portfolio item (GitHub/HF/Obsidian); unsupported claims are rejected by a validator (`web/src/lib/core/grounding/claims.ts`) rather than shown to the user. The `hallucinated-claim` scenario passes 10/10. The `unsupported-claim-gpa-employer` scenario (W2) is a known gap that fails 0/10 and is the only source of silent failures in the scripted verifier-on column (10 of 10); the verifier-on real-LLM column has 2 more silent failures (§4).
