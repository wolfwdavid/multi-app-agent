...
| `scene07-evals.mjs` | cwd `web/`: `node ../demo/tools/scene07-evals.mjs --scratch <dir> [--n 10]` (or set `SCENE_SCRATCH`) | Driver for scene 07. It runs `eval.ts --llm fake --no-artifacts --out <dir>/evals.json`, prints the methodology, replays the progress dots, then shows a per-scenario table condensed from that `evals.json` and the CLI's own TOTAL and silent-failure lines. |
